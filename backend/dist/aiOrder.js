...
exports.processAIOrder = processAIOrder;
...
exports.OrderRequestSchema = zod_1.z.object({
    ...
    cart: zod_1.z.array(zod_1.z.any()),
    history: zod_1.z.array(zod_1.z.any()),
});
...
Menu:
${(0, menu_1.getMenuSummary)()}

Valid item IDs: ${menu_1.menuItems.map((i) => i.id).join(", ")}
...
function matchItem(segment) {
    ...
    let bestMatch = null;
    let bestScore = 0;
    for (const item of menu_1.menuItems) {
...
function parseRemoveCommand(msg) {
    if (!msg.includes("remove") || msg.includes("remove everything"))
        return null;
...
function parseUpdateCommand(msg) {
    const changeMatch = msg.match(/(?:change|make|set|update)\s+(.+?)\s+(?:to|=)\s+(\d+|one|two|three|four|five)/);
    if (!changeMatch)
        return null;
    const itemPart = changeMatch[1];
    const qtyWord = changeMatch[2];
...
function extractItemsFromHistory(history) {
    const found = [];
...
function resolveFollowUp(msg, history) {
    const m = msg.toLowerCase();
    if (m.includes("suggest") || m.includes("recommend") || m.includes("what")) {
        return null;
    }
    const isFollowUp = m.includes("first") ||
        m.includes("second") ||
        m.includes("third") ||
        m.includes("yes") ||
        m.includes("sure") ||
...
function buildFallbackResponse(message, history = []) {
    ...
    if (msg.includes("menu") || msg.includes("what do you have") || msg.includes("what do you serve")) {
        return {
            reply: `We have ${menu_1.menuItems.length} items across Chef Picks, Mains, Sides, Drinks, and Dessert. What are you in the mood for?`,
            actions: [],
...
async function processAIOrder(message, cart, history) {
...
//# sourceMappingURL=aiOrder.js.map